import 'package:app/src/core/models/my_data_types.dart';
import 'package:app/src/features/home/domain/categories/residence.dart';
import 'package:app/src/core/models/opening_hours.dart';
import 'package:google_maps_flutter/google_maps_flutter.dart';

final List<Residence> testResidences = [
  Residence(
    id: 'res_001',
    categoryId: 1,
    subCategoryId: 1,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Cozy Apartment in Mirpur',
    cityName: 'Mirpur',
    sectorName: 'F1',
    geoLocation: LatLng(33.1840, 73.5186),
    avgRating: 4.5,
    totalReviews: 120,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 10),
    price: 25000,
    genderPref: GenderPreference.familyFriendly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_002',
    categoryId: 1,
    subCategoryId: 2,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Luxury Flat with Scenic View',
    cityName: 'Mirpur',
    sectorName: 'C4',
    geoLocation: LatLng(33.1885, 73.5112),
    avgRating: 4.8,
    totalReviews: 95,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 12),
    price: 40000,
    genderPref: GenderPreference.maleOnly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_003',
    categoryId: 2,
    subCategoryId: 1,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Affordable Hostel for Students',
    cityName: 'Mirpur',
    sectorName: 'D2',
    geoLocation: LatLng(33.1862, 73.5214),
    avgRating: 3.9,
    totalReviews: 60,
    isPopular: false,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 15),
    price: 8000,
    genderPref: GenderPreference.maleOnly,
    isFurnished: false,
  ),
  Residence(
    id: 'res_004',
    categoryId: 1,
    subCategoryId: 3,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Shared Apartment with Roommates',
    cityName: 'Mirpur',
    sectorName: 'B1',
    geoLocation: LatLng(33.1831, 73.5179),
    avgRating: 4.2,
    totalReviews: 85,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 18),
    price: 15000,
    genderPref: GenderPreference.femaleOnly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_005',
    categoryId: 2,
    subCategoryId: 2,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Girls Hostel with Facilities',
    cityName: 'Mirpur',
    sectorName: 'E3',
    geoLocation: LatLng(33.1893, 73.5167),
    avgRating: 4.7,
    totalReviews: 100,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 20),
    price: 12000,
    genderPref: GenderPreference.femaleOnly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_006',
    categoryId: 1,
    subCategoryId: 1,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Penthouse Living',
    cityName: 'Mirpur',
    sectorName: 'A2',
    geoLocation: LatLng(33.1857, 73.5144),
    avgRating: 4.9,
    totalReviews: 150,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 22),
    price: 60000,
    genderPref: GenderPreference.familyFriendly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_007',
    categoryId: 2,
    subCategoryId: 3,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Budget Hostel with Basic Needs',
    cityName: 'Mirpur',
    sectorName: 'F3',
    geoLocation: LatLng(33.1829, 73.5120),
    avgRating: 3.5,
    totalReviews: 45,
    isPopular: false,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 24),
    price: 7000,
    genderPref: GenderPreference.maleOnly,
    isFurnished: false,
  ),
  Residence(
    id: 'res_008',
    categoryId: 1,
    subCategoryId: 2,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Stylish Studio Apartment',
    cityName: 'Mirpur',
    sectorName: 'C1',
    geoLocation: LatLng(33.1875, 73.5191),
    avgRating: 4.3,
    totalReviews: 70,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 26),
    price: 28000,
    genderPref: GenderPreference.familyFriendly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_009',
    categoryId: 2,
    subCategoryId: 1,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Comfortable Hostel for Girls',
    cityName: 'Mirpur',
    sectorName: 'G2',
    geoLocation: LatLng(33.1869, 73.5155),
    avgRating: 4.4,
    totalReviews: 88,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 28),
    price: 11000,
    genderPref: GenderPreference.femaleOnly,
    isFurnished: true,
  ),
  Residence(
    id: 'res_010',
    categoryId: 1,
    subCategoryId: 3,
    coverImageUrl: 'assets/images/residence/image_1.jpg',
    name: 'Family Apartment with Parking',
    cityName: 'Mirpur',
    sectorName: 'H1',
    geoLocation: LatLng(33.1838, 73.5105),
    avgRating: 4.6,
    totalReviews: 110,
    isPopular: true,
    openingHours: _defaultOpeningHours,
    status: true,
    createdAt: DateTime(2024, 3, 30),
    price: 35000,
    genderPref: GenderPreference.familyFriendly,
    isFurnished: true,
  ),
];

final List<OpeningHours> _defaultOpeningHours = [
  OpeningHours(day: 'Monday', startTime: '9:00 AM', endTime: '9:00 PM'),
  OpeningHours(day: 'Tuesday', startTime: '9:00 AM', endTime: '9:00 PM'),
  OpeningHours(day: 'Wednesday', startTime: '9:00 AM', endTime: '9:00 PM'),
  OpeningHours(day: 'Thursday', startTime: '9:00 AM', endTime: '9:00 PM'),
  OpeningHours(day: 'Friday', startTime: '9:00 AM', endTime: '9:00 PM'),
  OpeningHours(day: 'Saturday', startTime: '9:00 AM', endTime: '9:00 PM'),
  OpeningHours(day: 'Sunday', startTime: '9:00 AM', endTime: '9:00 PM'),
];
