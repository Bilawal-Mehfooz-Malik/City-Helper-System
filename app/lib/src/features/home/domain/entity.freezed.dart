...
class Residence extends Entity {
  const Residence({required this.id, required this.categoryId, required this.subCategoryId, required this.coverImageUrl, required this.name, required this.cityName, required this.sectorName, @_latLngJsonConverter required this.latLng, this.avgRating = 0.0, this.totalReviews = 0, this.isPopular = false, @_openingHoursConverter final  Map<DayOfWeek, OpeningHours>? openingHours, this.entityStatus = OperationalStatus.defaultStatus, this.status = ApprovalStatus.pending, @_timestampJsonConverter required this.createdAt, this.type = EntityType.residence, @JsonKey(name: 'price') required this.pricing, this.isFurnished = false, this.genderPref = GenderPreference.familyFriendly, this.listingType = ListingType.forRent, this.timezone = "Asia/Karachi", final  Map<String, String> scheduledTaskNames = const {}, final  String? $type}): _openingHours = openingHours,_scheduledTaskNames = scheduledTaskNames,$type = $type ?? 'residence',super._();
